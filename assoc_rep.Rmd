...
```{r}
make_assoc_pairs <- function(vocab_aoa) {
  cue_target <- read.csv(paste(getwd(), "/in_files/association_pairs.csv", sep = ""))
  
  #Filter all the cues and targets that are not learnt after 30 months and are not normed
  cue_target <- cue_target %>%
    filter(cue %in% vocab_aoa$uni_lemma,
           target %in% vocab_aoa$uni_lemma) %>%
    filter(normed == "YES")
  ...
  assoc_link <- cue_target %>%
    ...
    left_join(vocab_aoa, c("pair.definition" = "uni_lemma")) %>%
    rename(pair = item, item.definition = target) %>%
    left_join(vocab_aoa, c("item.definition" = "uni_lemma")) %>%
